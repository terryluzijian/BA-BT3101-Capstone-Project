...
class DepartmentParser(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        file_path = parent_path + '/data/%s' % self.DATA
    ...
    def start_requests(self):
    	...
    	for index, value in self.faculty_data:
    		school = value['Name']
    		url = value['URL']
    		url_xpath = value['XPATH']
    		remark = value['remark']
    		sub_request = Request(url, self.parse)
    ...
    def parse(self, response):
    	...
    	if url_xpath == '-':
    		...
    		for url in url_list:
    			sub_request = Request(url, self.parse_link)
    			sub_request.meta['school'] = school
    			yield sub_request

    	# Parse the corresponding links and yield items
    	url_xpath_ex_href = url_xpath.replace('/@href', '')
    	link_element = self.generic_get_anchor_and_text(response, url_xpath_ex_href, url_xpath)
    	...
    	if remark == 'Pagination':
    		if response.url[-1] == 'A':
    			for index in range(1, 26):
	    			sub_request = Request(response.url[:-1] + '%c' % index, self.parse)
	    			sub_request.meta['school'] = school
	    			sub_request.meta['url_xpath'] = url_xpath
	    			sub_request.meta['remark'] = remark
	    			yield sub_request
    ...
    def parse_link(self, response):
        ...
        department['url'] = href
        department['school_name'] = school
        department['title'] = response.xpath('//title/text()').extract_first()
        yield department_item
