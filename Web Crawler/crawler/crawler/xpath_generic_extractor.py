...
The following varibales specifies nodes to select by XPath
...
through clickling on anchored links embedded in menu of the current page.
...
class GenericExtractor(object):
	...
	# User ancestor axis to filter out cetain tags optionally with cetain attributes
	...
	OTHERS_IMPLIED = 'not(ancestor::*[contains(@*, "banner")]) and not(ancestor::*[contains(@*, "crumb")]) and not(ancestor::*[contains(@*, "quick")])'
	COND_COMBINE = ' and '.join(frozenset([HEADER_IMPLIED, HEADER_TAGGED, 
	...
	def generic_get_anchor_and_text(self, response, content_xpath, href_xpath):
		...
		content_text = map(lambda htmlstring: ' '.join(html.fromstring(htmlstring).xpath(self.TEXT_XPATH)), content)
